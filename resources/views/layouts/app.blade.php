...
<body class="min-h-screen bg-[#f5f7f3] text-[#19251d] antialiased" style="font-family: 'Instrument Sans', sans-serif;">
    ...
    <div class="flex min-h-screen">
        {{-- Sidebar --}}
        <aside id="sidebar"
            class="fixed inset-y-0 left-0 z-30 w-64 -translate-x-full border-r border-[#e0e8df] bg-white px-5 py-6 transition-transform duration-300 lg:translate-x-0">
            {{-- Logo --}}
            <a href="{{ url('/') }}" class="flex items-center gap-2.5">
                ...
                <span class="text-lg font-bold tracking-[-0.04em]">nineteen<span
                        class="text-[#82b72d]">jobs</span></span>
            </a>
                    ...
                    @auth
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open"
                                class="flex items-center gap-2 rounded-full bg-[#f0f5ec] py-1.5 pl-1.5 pr-3 text-xs font-bold">
                                <div
                                    class="grid size-7 place-items-center rounded-full bg-[#1d3024] text-[10px] text-white font-bold">
                            ...
                            <div x-show="open" @click.away="open = false" x-transition
                                class="absolute right-0 mt-2 w-48 rounded-xl border border-[#e0e8df] bg-white p-2 shadow-lg z-50">
                                <a href="{{ route('profile.edit') }}"
                                    class="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-[#647066] hover:bg-[#f5f8f2]">Profil
                                    Saya</a>
                                <hr class="my-1 border-[#e8ede6]">
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit"
                                        class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-red-500 hover:bg-red-50">Keluar</button>
                                </form>
                            </div>
                        </div>
                    @else
                        <a href="{{ route('login') }}"
                            class="rounded-full bg-[#b7f34a] px-4 py-2 text-xs font-bold text-[#182516] hover:bg-[#a5e839] transition">Masuk</a>
                    @endauth
                </div>
            </header>
